...
extern void lyra2vc0ban_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNonce, uint64_t *d_outputHash);

extern void lyra2vc0ban_cpu_init(int thr_id, uint64_t* matrix);
...
int scanhash_lyra2vc0ban(int thr_id, uint32_t *pdata,
	...
	uint32_t *hashes_done)
{
	...
	{ 
		...
		lyra2vc0ban_cpu_init(thr_id, d_hash2);
	...
	do {
		...
		lyra2vc0ban_cpu_hash_32(thr_id, throughput, pdata[19], d_hash);
		...
		if(foundNonce[0] != 0)
		{
			...
			if(opt_verify)
			{
				...
				lyra2vc0ban_hash(vhash64, endiandata);
			}
