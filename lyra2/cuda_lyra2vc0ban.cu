...
void lyra2vc0ban_gpu_hash_32_2(uint32_t threads, uint32_t startNounce, uint2 *outputHash)
{
...
__host__
void lyra2vc0ban_cpu_init(int thr_id, uint64_t *d_matrix)
{
...
__host__
void lyra2vc0ban_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint64_t *g_hash)
{
	int dev_id = device_map[thr_id % MAX_GPUS];
	...
	if (device_sm[dev_id] == 700)
		cudaFuncSetAttribute(lyra2vc0ban_gpu_hash_32_2, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
#endif

	lyra2vc0ban_gpu_hash_32_1 << <grid2, block2, 0, gpustream[thr_id] >> > (threads, startNounce, (uint2*)g_hash);
	CUDA_SAFE_CALL(cudaGetLastError());

	lyra2vc0ban_gpu_hash_32_2 << <grid1, block1, sm, gpustream[thr_id] >> > (threads, startNounce, (uint2*)g_hash);
	CUDA_SAFE_CALL(cudaGetLastError());

	lyra2vc0ban_gpu_hash_32_3 << <grid2, block2, 0, gpustream[thr_id] >> > (threads, startNounce, (uint2*)g_hash);
	CUDA_SAFE_CALL(cudaGetLastError());
